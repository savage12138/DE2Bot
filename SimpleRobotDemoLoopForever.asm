...
GoStraight:				 	; Go straight with FMid speed and current direction
	LOAD	Ten				; Load 10
	OUT		SSEG2			; Output to SSEG2
	LOAD	FMid			; Load speed value
	STORE	DVel			; Store to DVel
	IN		THETA			; Read current theta
	STORE	DTheta			; Store to DTheta
	CALL	ControlMovement	; Moving
	JUMP	GoStraight		; Loop

; Timer counter	
NCNT:		DW 3			; Counter
CCNT:       DW 90			; Circling timer
OLDVAL:		DW 0			; Initialize old timer value to 0
RCNT:       DW 10			; Reverse timer
; Circling Subroutine
Circling:
	IN 		TIMER			; Load timer
	STORE   OLDVAL			; Store old timer value
REVERSELOOP:				; Reverse/Moving backward function
	LOADI   -200			; Sets wheel speed
	OUT     LVELCMD			; Output to left wheel
	OUT     RVELCMD			; Output to right wheel
	IN      TIMER			; Load timer
	SUB     OLDVAL			; Subtract old timer value
	SUB     RCNT			; Subtract reverse function timer value
	JNEG    REVERSELOOP		; Move backward until timer ends
	IN      TIMER			; Load timer
	STORE   OLDVAL			; Store in old timer value
CIRCLELOOP:					; Circling movement function
	IN		DIST5			; Sonar 5 distance reading
	SUB		Ft1				; Check if it is less than 1 ft
	JNEG	DoLarge			; Make the circling larger
	IN		DIST4			; Sonar 4 distance reading
	SUB		Ft1				; Check if it is less than 1 ft
	JNEG	DoVeryLarge		; Make the circling larger
	IN		DIST5			; Sonar 5 distance reading
	SUB		Ft2				; Check if it is greater than 2 ft
	JPOS	DoSmall			; Make the circling smaller
	LOAD	Eight			; Load 8
	OUT		SSEG2			; Output to SSEG2
	LOADI   510				; Set left wheel speed
	OUT     LVELCMD			; Output to left wheel
	LOADI	285				; Set Right wheel speed
	OUT		RVELCMD			; Output to right wheel
	JUMP	CIRCLELOOP		; Loop
